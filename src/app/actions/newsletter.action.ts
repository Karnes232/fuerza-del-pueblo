...
export async function addNewsletterSubscriber(email: string): Promise<{
  ...
  message: string
    ...
    const { data: existingRecords, error: checkError } = await createClient(cookieStore)
    ...
    if (checkError) {
        console.error("Error al verificar si el correo ya está suscrito.", checkError)
        return {
          success: false,
    ...
    const { error } = await createClient(cookieStore).from("newsletter_subscribers").insert({ email })
